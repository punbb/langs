...
'Database user pass'		=>	'Имя пользователя и пароль пользователя БД:',
...
'Database username info'	=>	'Введите имя пользователя и пароль, используемые для подключения к БД. Для SQLite данный параметр не требуется.',
'Table prefix info'			=>	'Необязательный параметр — префикс для таблиц БД. Определив префикс (например, <em>foo_</em>), можно использовать несколько копий PunBB с одной БД.',
...
// Install errors
'No database support'		=>	'Ваша сборка PHP не поддерживает БД, совместимой с PunBB. Для продолжения установки необходима поддержка хотя бы одной - MySQL, PostgreSQL или SQLite.',
...
'Invalid email'				=>	'Введенный e-mail адрес администратора неверен. Пожалуйста, вернитесь и исправьте ошибку.',
'Missing base url'			=>	'Вы должны указать базовый URL. Пожалуйста, вернитесь и исправьте ошибку.',
'No such database type'		=>	'\'%s\' неверный тип БД.',
...
'PunBB already installed'	=>	'Таблица «%1$susers» уже существует в БД «%2$s». Это может означать, что PunBB уже установлен или установлено какое-то другое ПО, использующее одну или несколько таблиц, необходимых для работы PunBB. Если вы хотите установить несколько копий PunBB в одну БД, то вы должны указать другой префикс.',
'Invalid language'          =>  'Выбранный языковой пакет не существует или поврежден.',

// Used in the install
'Default language'          =>  'Язык по умолчанию',
'Default language help'     =>  '(Если вы удалите языковой пакет, обновите этот параметр)',
'Default announce heading'	=>	'Пример объявления',
'Default announce message'	=>	'<p>Введите текст вашего объявления здесь.</p>',
'Default maint message'		=>	"Форум временно закрыт на профилактику. Пожалуйста, попробуйте зайти через несколько минут.<br />\\n<br />\\n/Администратор",
...
'Warning'					=>	'Внимание!',
'No cache write'			=>	'<strong>Каталог cache недоступен для записи!</strong> Для корректной работы PunBB каталог под названием <em>cache</em> должен быть доступен для записи. Используйте chmod, чтобы задать права доступа для каталога. Если сомневаетесь, установите chmod на 0777.',
'No avatar write'			=>	'<strong>Каталог avatar недоступен для записи!</strong> Если вы хотите, чтобы пользователи могли загружать собственные аватары, то должны убедиться, что каталог под названием <em>img/avatars</em> доступен для записи. Позже вы сможете указать другую папку для хранения аватар (смотрите Администрирование/Настройки/Дополнительные возможности). Используйте chmod, чтобы задать права доступа для каталога. Если сомневаетесь, установите chmod на 0777.',
