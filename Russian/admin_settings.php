...
'User has posted label'			=>	'Отображать точку перед индикатором состояния темы, если пользователь отвечал в ней ранее. Отключите функцию, если нагрузка на сервера высока. Не поддерживается для SQLite',
...
'New reg info'					=>	'Можно использовать проверку всех новых регистраций. Когда проверка регистраций включена, пользователь получает на e-mail сообщение со ссылкой активации. Он может воспользоваться ею для авторизации. Эта функция также требует от пользователя подтверждать адрес e-mail, если он захочет его сменить после регистрации. Этот эффективный путь ограничения пустых регистраций позволяет быть уверенным, что у всех пользователей в профиле указан реальный адрес e-mail.',
...
'Report new reg'				=>	'Оповещение по e-mail',
...
'Error invalid web e-mail'		=>	'Введенный E-mail адрес веб-мастера содержит ошибку.',
