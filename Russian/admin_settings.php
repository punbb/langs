...
'URL scheme info'				=>	'<strong>ВНИМАНИЕ!</strong> Если вы выберите схему, отличную от базовой, вы должны скопировать/переименовать файл <em>.htaccess.dist</em> в <em>.htaccess</em> в корне форума. Сервер, на котором установлен форум, должен быть сконфигурирован с поддержкой mod_rewrite и должен разрешать использование файлов <em>.htaccess</em>. Если вы используете веб-сервер, отличный от Apache, пожалуйста обратитесь к документации вашего сервера.',
...
'Avatar Max size help'			=>	'Максимальный размер в байтах (рекомендуется значение 10,240).',
...
'Features gzip info'			=>	'Если включено, PunBB будет передавать вашему браузеру данные, сжатые gzip. Это сократит нагрузку на полосу пропускания данных, но немного увеличит нагрузку на процессор (CPU). Для этой функции нужно, чтобы PHP был сконфигурирован с поддержкой zlib (--with-zlib). Замечание: Если вы уже используете один из Apache-модулей: mod_gzip или mod_deflate, установленных для сжатия PHP сценариев, вам стоит отключить данную функцию.',
...
'Enable announcement label'		=>	'Выводить сообщение объявления.', // перефразировать
...
'New reg info'					=>	'Вы можете использовать проверку всех новых регистраций. Когда проверка регистраций включена, пользователь получает на e-mail сообщение со ссылкой активации. Он может воспользоваться ею для авторизации. Эта функция также требует от пользователя подтверждать адрес e-mail, если он захочет его сменить после регистрациито. Это эффективный путь ограничить пустые регистрации и быть уверенным, что у всех пользователей в профиле указан реальный адрес e-mail.',
...
'Allow banned label'			=>	'Разрешить регистрацию с адресом e-mail, который находится в чёрном списке (заблокирован).',
